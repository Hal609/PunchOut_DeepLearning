...
import time
from utils import ram_dict_from_csv
...
class NESWindow():
    def __init__(self, rom_path, font_path, headless=False, show_debug=True):
    ...
    def render_text(self, renderer, font, text, x, y, line_height=20, max_length=75):
        ...
        for i, line in enumerate(wrapped_lines):
            ...
            if surface is None:
                print(f"SDL2_ttf error: {sdl2.SDL_GetError().decode('utf-8')}")
                continue
    ...
    def step(self):
        if self.show_debug: 
            ...
            self.perform_inputs()
            # self.nes.controller = self.inputs
            ...
            if current_time - self.last_debug_update >= 0.2:
